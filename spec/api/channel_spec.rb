    ...
    it "sends requests" do
      expect(Rubycord::API).to receive(:request)
        .with(
          ...
          channel_id,
          :get,
          "#{Rubycord::API.api_base}/channels/#{channel_id}/messages/#{message_id}/reactions/test?limit=27&before=#{before_id}&after=#{after_id}",
          any_args
        )
    ...
    it "percent-encodes emoji" do
      expect(Rubycord::API).to receive(:request)
        .with(
          ...
          channel_id,
          :get,
          "#{Rubycord::API.api_base}/channels/#{channel_id}/messages/#{message_id}/reactions/%F0%9F%91%8D?limit=27&before=#{before_id}&after=#{after_id}",
          any_args
        )
    ...
    it "uses the maximum limit of 100 if nil is provided" do
      expect(Rubycord::API).to receive(:request)
        .with(
          ...
          channel_id,
          :get,
          "#{Rubycord::API.api_base}/channels/#{channel_id}/messages/#{message_id}/reactions/test?limit=100&before=#{before_id}&after=#{after_id}",
          any_args
        )
