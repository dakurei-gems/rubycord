...
describe Rubycord::Overwrite do
  describe "#initialize" do
    context "when object is an Integer" do
      ...
      it "accepts the API value for `type`" do
        ...
        expect(overwrite.type).to eq Rubycord::Overwrite::TYPES.key(0)
      end
    ...
    context "when object is a User type" do
      let(:user_types) { [Rubycord::User, Rubycord::Member, Rubycord::Recipient, Rubycord::Profile] }
      let(:users) do
    ...
    context "when object is a Role" do
      let(:role) { instance_double(Rubycord::Role) }

      it "infers type from a Role object" do
        allow(Rubycord::Role).to receive(:===).with(anything).and_return(false)
        allow(Rubycord::Role).to receive(:===).with(role).and_return(true)
  ...
  describe "#to_hash" do
    let(:id) { instance_double(Integer) }
    let(:allow_perm) { instance_double(Rubycord::Permissions, bits: allow_bits) }
    let(:allow_bits) { instance_double(Integer) }
    let(:deny_perm) { instance_double(Rubycord::Permissions, bits: deny_bits) }
    let(:deny_bits) { instance_double(Integer) }

    before do
      allow(allow_perm).to receive(:is_a?).with(Rubycord::Permissions).and_return(true)
      allow(deny_perm).to receive(:is_a?).with(Rubycord::Permissions).and_return(true)
    end
    ...
    it "creates a hash from the relevant values" do
      ...
      expect(overwrite.to_hash).to eq({
        id: id,
        type: Rubycord::Overwrite::TYPES[:member],
        allow: allow_bits,
        deny: deny_bits
      })
    end
  ...
  describe ".from_hash" do
    let(:id) { instance_double(Integer) }
    let(:type) { Rubycord::Overwrite::TYPES[:role] }
